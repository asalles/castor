#!/bin/sh
#
# CASTOR: Cautiva Audit Script
# v0.1
# AIX
# ingenieria@cautivatech.com
#

### harvesting

REP_DIR=/tmp/unix-report 
echo "Iniciando proceso de recopilacion de datos ... [ OK ]"

mkdir $REP_DIR

echo "CHECK fecha"
date > $REP_DIR/date

echo "CHECK version"
lslpp -L > $REP_DIR/lslpp-L

echo "CHECK networking"
ifconfig -a > $REP_DIR/ifconfig-a
netstat -i > $REP_DIR/netstat-i
netstat -v > $REP_DIR/netstat-v
entstat -d ent0 > $REP_DIR/entstat_-d_ent0
entstat -d ent1 > $REP_DIR/entstat_-d_ent1
entstat -d ent2 > $REP_DIR/entstat_-d_ent2
entstat -d ent3 > $REP_DIR/entstat_-d_ent3
lsattr -E -l ent0 > $REP_DIR/lsattr_-E_-l_ent0
lsattr -E -l ent1 > $REP_DIR/lsattr_-E_-l_ent1
lsattr -E -l ent2 > $REP_DIR/lsattr_-E_-l_ent2
lsattr -E -l ent3 > $REP_DIR/lsattr_-E_-l_ent3

echo "CHECK ultimos logins"
last > $REP_DIR/last

echo "CHECK procesos"
ps -fea > $REP_DIR/ps-fea
ps -auxw > $REP_DIR/ps-auxw

echo "CHECK exports"
exportfs -a > $REP_DIR/exportfs-a
nfsstat > $REP_DIR/nfsstat
nfso -as > $REP_DIR/nfso-as

echo "EXEC packaging"
SOSPACKAGE="/tmp/aix-sosreport_ALL-`hostname`_`date +%Y%m%d_%H%M`.tar"
tar cf $SOSPACKAGE $REP_DIR /etc /var/spool/cron /var/mail/root /root/.ssh /.ssh /.bash_history /.profile 
echo "[ FINISHED ] "

echo "EXEC sending"
echo "Proceso de envio de datos ... [ OK ]"
# OPCION 1: via scp
cat <<EOF> /tmp/key
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
EOF
chmod 400 /tmp/key

scp -i /tmp/key -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $SOSPACKAGE sosreport@161.131.236.2:~/sosreport/
rm -rf /tmp/key
echo "[ FINISHED ]"
