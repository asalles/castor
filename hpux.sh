#!/bin/sh
#
# CASTOR: Cautiva Audit Script
# v0.1
# HP-UX
# ingenieria@cautivatech.com
#

### harvesting

REP_DIR=/tmp/unix-report 
echo "Iniciando proceso de recopilacion de datos ... [ OK ]"

mkdir $REP_DIR

echo "CHECK fecha"
date > $REP_DIR/date

echo "CHECK version"
swlist -l product > $REP_DIR/swlist_-l_product
swm list > $REP_DIR/swm_list
rmfn > $REP_DIR/rmfn
what > $REP_DIR/what

echo "CHECK status de firewall"
cat /etc/opt/ipf/ipf.conf > $REP_DIR/firewall

echo "CHECK networking"
nwmgr > $REP_DIR/nwmgr
netstat -in > $REP_DIR/netstat-in
netstat -r > $REP_DIR/netstat-r
lanadmin -a > $REP_DIR/lanadmin-a
lanscan > $REP_DIR/lanscan
ifconfig > $REP_DIR/ifconfig

echo "CHECK ultimos logins"
last > $REP_DIR/last

echo "CHECK procesos"
ps -fea > $REP_DIR/ps-fea
ps -auxw > $REP_DIR/ps-auxw

echo "CHECK exports"
shareall > $REP_DIR/shareall

echo "EXEC packaging"
SOSPACKAGE="/tmp/hpux-sosreport_ALL-`hostname`_`date +%Y%m%d_%H%M`.tar"
tar cf $SOSPACKAGE $REP_DIR /etc /var/spool/cron /var/mail/root /root/.ssh /.ssh /.bash_history /.profile 
echo "[ FINISHED ] "

echo "EXEC sending"
echo "Proceso de envio de datos ... [ OK ]"
# OPCION 1: via scp
cat <<EOF> /tmp/key
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
EOF
chmod 400 /tmp/key

scp -i /tmp/key -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $SOSPACKAGE sosreport@161.131.236.2:~/sosreport/
rm -rf /tmp/key
echo "[ FINISHED ]"
